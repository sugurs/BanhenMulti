...
def train_test():
    ...
    for epoch in range(cfg.max_train_epochs):
        ...
        if test_acc > best_acc:
            ...
            save_path = '/media/D_4TB/SUGURS/Banhen_multi/save_models_clssify_only/model_epoch_%d_acc_all_%0.3f.pth'%(epoch + 1, test_acc)
            torch.save(net, save_path)

        print('[epoch %d] train_loss: %.3f,  best_acc: %.3f, best_acc_list: [%.3f, %.3f, %.3f, %.3f], current_acc_list: [%.3f, %.3f, %.3f, %.3f], '
              'current_test_acc: %.3f' %
              (epoch + 1, running_loss / train_steps, best_acc, best_acc_list[0], best_acc_list[1], best_acc_list[2], best_acc_list[3],
               acc_banhenai, acc_banhengeda, acc_weisuoxing, acc_zengshengxing, test_acc))
        for param_group in optimizer.param_groups:
